...
It's also possible to trigger a manual GitLab-QA pipeline against a specific [GitLab environment](/handbook/engineering/infrastructure-platforms/developer-experience/end-to-end-pipeline-monitoring/) using the `RELEASE` and `QA_IMAGE` variable from the `test-on-omnibus` job of GitLab's Merge Request.
