...
This page outlines steps for investigating [end-to-end test](https://docs.gitlab.com/development/testing_guide/end_to_end/index/) test failures, deployments, and troubleshooting various GitLab environments. It provides information on how to handle failures, identifying the problematic commit, the logging tools, and ways to reproduce the failure.
...
- [`test-on-omnibus`](https://docs.gitlab.com/development/testing_guide/end_to_end/#using-the-test-on-omnibus-job) runs the `full` suite of end-to-end tests against an omnibus Docker image built from `master`
- [`test-on-gdk`](https://docs.gitlab.com/development/testing_guide/end_to_end/#using-the-test-on-gdk-job) runs the full suite of end-to-end tests as part of the `gdk-instance` job against a GDK instance from a Docker image built from `master`
...
You will [need access to the https://ops.gitlab.net/gitlab-com/chatops](https://docs.gitlab.com/development/chatops_on_gitlabcom/#requesting-access) project to run `/chatops` commands.
...
You can also determine if your commit has been deployed on a GitLab environment using [ChatOps](https://docs.gitlab.com/ci/chatops/).
...
or by calling the [`/api/v4/version` API](https://docs.gitlab.com/api/version/#version-api) where it is displayed as a value of the `revision` attribute.
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Stack trace](https://docs.gitlab.com/ci/quick_start/#viewing-the-status-of-your-pipeline-and-jobs)                                                                                                                                                                                    | Shown in the job's log; the starting point for investigating the test failure                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [Screenshots and HTML captures](https://docs.gitlab.com/ci/jobs/job_artifacts/#browsing-artifacts)                                                                                                                                                                                 | Available for download in the job's artifact for up to 1 week after the job run                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [QA Logs](https://docs.gitlab.com/ci/jobs/job_artifacts/#browsing-artifacts)                                                                                                                                                                                                       | Included in the job's artifacts; valuable for determining the steps taken by the tests before failing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| [System Logs](https://docs.gitlab.com/administration/logs/) (gitlab-rails, sidekiq, etc.)                                                                                                                                                                                              | Included in the job's artifacts for containerized test runs, such as master and nightly. These are useful for investigating errors originating from the GitLab application itself. <br/><br />  A summary of the system logs related to a test failure can also be found in the description of QA failure issues generated from master and nightly runs containing a correlation ID.                                                                                                                                                                                                                                          |
...
Note that to be able to pull the docker image from `registry.gitlab.com` you need to [authenticate with the Container Registry](https://docs.gitlab.com/user/packages/container_registry/#authenticate-with-the-container-registry).
...
- Additional information about running tests locally can be found in the [QA readme](https://gitlab.com/gitlab-org/gitlab/tree/master/qa#running-specific-tests) and in the [instructions for running tests that require special setup](https://docs.gitlab.com/development/testing_guide/end_to_end/running_tests_that_require_special_setup/#jenkins-spec).
- To determine if the test is [flaky](https://docs.gitlab.com/development/testing_guide/flaky_tests/#whats-a-flaky-test), check the logs or run the test a few times. If it passes at least once but fails otherwise, it's flaky.
...
You can find some [general tips for troubleshooting problems with GitLab end-to-end tests in the development documentation](https://docs.gitlab.com/development/testing_guide/end_to_end/troubleshooting/).
