...
Stage groups can follow the [developer documentation](https://docs.gitlab.com/development/application_slis/rails_request/) to set target durations and should ask for a review from a Scalability Team Member on the MR.
...
For now, Error budgets will continue to use the legacy SLI using a 5 second target duration until the stage group chooses to [opt-in](https://docs.gitlab.com/development/application_slis/rails_request/#error-budget-attribution-and-ownership) to using the customizable durations. For customizable durations the default threshold (for endpoints _without_ a specific target) is set to 1 second.
