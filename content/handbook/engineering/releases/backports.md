...
- Backports are delivered according to the [Maintenance Policy](https://docs.gitlab.com/policy/maintenance/) and backport exceptions out of that policy are delivered on a best-effort basis and not guaranteed
...
You can check the [Maintenance Policy](https://docs.gitlab.com/policy/maintenance/) to see what is covered
...
At GitLab, we backport security fixes and bug fixes according to the [Maintenance Policy](https://docs.gitlab.com/policy/maintenance/).

### Is that the same as a Patch Release?

No it's not. Backporting is a catch-all term for any activity that applies updates or patches from a newer version of software to an older version. At GitLab we have a specific [Patch Release process](/handbook/engineering/releases/#patch-releases-overview) that is applied according to the [Maintenance Policy](https://docs.gitlab.com/policy/maintenance/). This is one of the release methods we use to ship self-managed, along with the security release and regular monthly release. There is a separate [exception process](https://docs.gitlab.com/policy/maintenance/#backporting-to-older-releases) for backports that are outside the scope of our maintenance policy and these are delivered on a best effort basis and not guaranteed.
...
If your backport is a security fix and covered by the maintenance policy, please follow the [Security Release Process](https://docs.gitlab.com/policy/maintenance/#security-releases).
