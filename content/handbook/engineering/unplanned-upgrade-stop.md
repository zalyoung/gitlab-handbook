...
An unplanned upgrade stop happens when we fail to communicate the necessity of this upgrade stop in our [upgrade path](https://docs.gitlab.com/update/#upgrade-paths). For more information, read what an [unplanned upgrade stop is](https://docs.gitlab.com/development/avoiding_required_stops/).

The most common cause for unplanned upgrade stops is Database migration errors. For example, [Migration errors in 16.[012]](https://gitlab.com/gitlab-org/gitlab/-/issues/423895). Other common causes of required stops listed in [Avoiding required stops documentation](https://docs.gitlab.com/development/avoiding_required_stops/#causes-of-required-stops).
...
1. DRI to identify if fix can be backported. If backporting is not possible, collaborate with the Distribution team to update the upgrade path and [retroactively add the unplanned stop](https://docs.gitlab.com/development/avoiding_required_stops/#retroactively-adding-required-stops) for immediate resolution.
