...
1. **In the event of an incident which has been triaged and confirmed as a clear Severity 1 impact:**
    1. Notify Infrastructure Leadership by typing `/incident escalate` in Slack. In the `On-call teams` drop-down menu select `dotcom leadership escalation` with appropriate message in the `Notification Message`.This notification should happen 24/7.
...
1. If the incident is Sev 1/2 , A [post-incident review](/handbook/engineering/infrastructure/incident-review/) is required to be done by the IM , Post-mortem tasks are notified in incident slack channel once the incident is marked as resolved. The DRI role for this can be delegated.
