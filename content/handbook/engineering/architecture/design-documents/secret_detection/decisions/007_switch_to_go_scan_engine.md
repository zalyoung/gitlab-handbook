...
2. There was an active development of SPP feature by vendoring SD Ruby gem so the migration would've pushed the SPP feature release further. Given the strict release timeline we had, we couldn't have afford it. We needed a non-disruptive approach.

3. Runway did not support(still) deployment to Self-Managed and Dedicated environments. Cloud Connector was an effortful option to consider. Runway [had plans](https://gitlab.com/gitlab-com/gl-infra/platform/runway/team/-/issues/236) to support it so a temporary approach of reusing the on-going work (vendor gem) made more sense.
...
4. Missing first-class support for generating efficient binary executable makes it difficult to unify the Secret Scanning engine across the Secret Detection suite.
...
2. We did not explore other Go regex alternatives during thr Spike since we were convinced using Ruby due to other constraints mentioned above. There could have been a possibility that third-party Go regex libraries perform better.
...
6. Adding cherry on top, integration with [Cloud Connector for SDS](https://gitlab.com/gitlab-org/gitlab/-/work_items/525472) wouldn't have a blocker of missing auth SDK, like we currently have for Ruby.
