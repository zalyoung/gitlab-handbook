...
[authentication tokens](https://docs.gitlab.com/security/tokens/#runner-authentication-tokens)
...
| Token type | Behavior |
| ---------- | -------- |
| [Registration token](https://docs.gitlab.com/security/tokens/#runner-authentication-tokens) | Leverages the `POST /api/v4/runners` REST endpoint to create a new runner, creating a new entry in `config.toml` and a `system_id` value in a sidecar file if missing (`.runner_system_id`). |
| [Runner authentication token](https://docs.gitlab.com/security/tokens/#runner-authentication-tokens) | Leverages the `POST /api/v4/runners/verify` REST endpoint to ensure the validity of the authentication token. Creates an entry in `config.toml` file and a `system_id` value in a sidecar file if missing (`.runner_system_id`). |
...
The [`CiRunner` type](https://docs.gitlab.com/api/graphql/reference/#cirunner) closely reflects the
...
[clean up stale runners](https://docs.gitlab.com/ci/runners/runners_scope/#clean-up-stale-group-runners) needs
...
[`POST /user/runners` REST API endpoint](https://docs.gitlab.com/api/users/#create-a-runner-linked-to-a-user).
These endpoints are only available to users that are
[allowed](https://docs.gitlab.com/user/permissions/#gitlab-cicd-permissions) to create runners at the specified
scope.
...
| GitLab Rails app | `%15.9` | Implement the `create_runner_machine` [feature flag](https://docs.gitlab.com/administration/feature_flags/). |
...
| GitLab Rails app | `%15.9` | Implement the `create_runner_workflow_for_admin` [feature flag](https://docs.gitlab.com/administration/feature_flags/). |
...
| GitLab Rails app | `%16.1`   | Make the [`POST /api/v4/runners`](https://docs.gitlab.com/api/runners/#create-a-runner) endpoint return `HTTP 410 Gone` permanently if either `allow_runner_registration_token` setting disables registration tokens. The Runners API v5 should return `HTTP 404 Not Found`. |
...
| GitLab Rails app |           | Document gotchas when [automatically rotating runner tokens](https://docs.gitlab.com/ci/runners/configure_runners/#automatically-rotate-runner-authentication-tokens) with multiple machines. |
...
Follow [the user documentation](https://docs.gitlab.com/ci/runners/new_creation_workflow/).
