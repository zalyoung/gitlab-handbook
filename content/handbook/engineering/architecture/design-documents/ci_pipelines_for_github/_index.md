...
GitHub will communicate with GitLab via our GitHub Actions with an OIDC token.
...
Each user that wants to run a pipeline on GitLab must first connect their GitLab account with GitHub
...
1. User configures a GitHub action workflow using our GitHub Action component
    ...
    1. They'll need to correctly set-up and pass secret variables to our component
...
1. This GitHub action using our component will make an API call to our GitLab server with
    ...
    3. The `sha, ref, commit_message, repo_url, repo_id` parameters
...
There's talks of using OIDC, but this doesn't quite fit the use case as we need to additionally map GitLab <-> GitHub users instead of just trusting GitHub users.
