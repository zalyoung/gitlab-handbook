...
| **CDotPlanStatus__c** | `ProductRatePlan` | Yes | String | `active`, `eoa_deprecated`, `tier_deprecated`, `not_applicable` | Represents the lifecycle stage of a plan: <br>• `active`: Currently salable and fully supported / available plans<br>• `eoa_deprecated`: Historical plans maintained only for existing subscriptions in the EoA offering<br>• `tier_deprecated`: Historical plans maintained only for existing subscriptions in after Tier rename<br>• `not_applicable`: Special cases where status concept doesn't apply |
