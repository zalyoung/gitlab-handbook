...
2. If the user has no external identity provider configured, the password field appears. 
3. If the user has an external identity provider, they are 
    a. redirected to the appropriate third-party identity provider
    b. redirected to the Organization-specific sign-in page
...
2. The user provides the path for their organization - `acme-corp`, for example. 
3. The user is redirected to the Organization-specific URL and is presented with all the authentication mechanisms enabled for that Organization.
