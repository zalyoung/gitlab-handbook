...
Every day, Monday-Friday, we run an automation to check for offboarding issues that have been open longer than 90 days. If any are found, we leave a comment with the [`/close` quick action](https://docs.gitlab.com/user/project/quick_actions/) to close the issue.
...
We leverage the [issues API](https://docs.gitlab.com/api/issues/#list-issues) to search for related issues matching the following filters:
