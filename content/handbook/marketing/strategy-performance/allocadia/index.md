...
3. Create a Line Item with a naming convention: `MDF 000  Partner Name  Details Related to Spend`
   Note, 000 is a placeholder for when once MDF is created, and this will automatically be updated once the MDF Request has been selected in the Salesforce campaign.
...
- The update is only applicable to the following fields:
    ...
    1. Issue description – only updates the Start Date / End Date (if they are changed)
    1. Tactic/event type
    1. Allocadia Line Item ID
    1. Estimated pipeline created (USD)
    1. Target number of contacts
    1. Expected number of deal registrations
    1. Campaign Name
