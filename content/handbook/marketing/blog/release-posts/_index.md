...
`workflow::verification` and `workflow::production` are being included while we adopt the [addition of the `workflow::complete` label](/handbook/product-development/how-we-work/). `workflow::complete` is the desired label to indicate an issue is considered by users to be delivered.
