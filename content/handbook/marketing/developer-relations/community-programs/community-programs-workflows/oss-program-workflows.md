...
GitLab Customers Portal contains the application form to request an open source GitLab license.
The workflow in this form is divided into two types of submission:

### An open source project is located on the GitLab.com
...
* Each project inside the namespace includes an OSI-approved open source license.
...
For more information about OSI-approved license check [see GitLab Docs](https://docs.gitlab.com/ee/subscriptions/community_programs.html#gitlab-for-open-source).

### An open source project is located on a Self-managed instance

Applicants provide basic information about their project with publicly accessible links. Information may highlight **why they need a self-managed solution.** In up to five days, an applicant receives a response with further instructions.

The form also includes the following statement, added with help from GitLab's legal team:
...
When applicants submit the Self-managed form to GitLab for the Open Source Program, those forms enter a review queue.
Note that:
...
Any individual can apply through the form while they fail verification. For the Self-managed use case, an applicant can send a request once per day.
