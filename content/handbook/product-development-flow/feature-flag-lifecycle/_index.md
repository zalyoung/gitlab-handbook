...
as the [feature flags offered as part of the product](https://docs.gitlab.com/development/feature_flags/).
...
**NOTE**:
[Feature flags are not supported in GitLab Dedicated](https://docs.gitlab.com/development/enabling_features_on_dedicated/#feature-flags).
...
Some of the benefits of using [development-type](https://docs.gitlab.com/development/feature_flags/#development-type) feature flags are:
...
1. [Developing with feature flags](https://docs.gitlab.com/development/feature_flags/): Learn about the types of
  feature flags, their definition and validation, how to create them, frontend and
  backend details, and other information.
1. [Documenting features deployed behind feature flags](https://docs.gitlab.com/development/documentation/feature_flags/):
...
1. [Controlling feature flags](https://docs.gitlab.com/development/feature_flags/controls/): Learn the process for deploying
...
1. [Conducting an experiment with feature flags](https://docs.gitlab.com/development/experiment_guide/): Learn how the Growth department uses experiment feature flags.

For information about how the user will interact with features behind flags, see:

1. [How GitLab administrators can enable and disable features behind flags](https://docs.gitlab.com/administration/feature_flags/):
  An explanation for GitLab administrators about how they can
  enable or disable GitLab features behind feature flags.
1. [What "features deployed behind flags" means to the GitLab user](https://docs.gitlab.com/user/feature_flags/):
...
1. Decide how you will implement the feature flag and its rollout according to:
   1. Choose the feature flag [type](https://docs.gitlab.com/development/feature_flags/#types-of-feature-flags).
   1. Decide on [the definition](https://docs.gitlab.com/development/feature_flags/#feature-flag-definition-and-validation) and plan out the YAML.
   1. Decide on what type of [actor](https://docs.gitlab.com/development/feature_flags/#feature-actors) to bind your feature flag to (if any).
...
1. Create a feature flag definition in code and make a follow-up rollout issue:
   1. [Create a feature flag](https://docs.gitlab.com/development/feature_flags/#create-a-new-feature-flag).
   1. [Create a feature flag rollout issue using the template](https://docs.gitlab.com/development/feature_flags/#development-type) (if necessary).
...
1. Add the feature flag into the code for the backend, frontend, and in tests:
   1. [Develop with the feature flag](https://docs.gitlab.com/development/feature_flags/#develop-with-a-feature-flag) in your vertical slice.
   1. [Include the feature flag in tests](https://docs.gitlab.com/development/feature_flags/#feature-flags-in-tests) to check its behavior when enabled and disabled.
   1. [Flip the feature flag locally](https://docs.gitlab.com/development/feature_flags/#enabling-a-feature-flag-locally-in-development) to ensure it works.
1. Add the feature behind the flag to the codebase through an MR following the [implementation process](https://docs.gitlab.com/development/feature_flags/).
...
1. Ensure you follow the [documentation guidelines for feature flags](https://docs.gitlab.com/development/documentation/feature_flags/) and keep them up-to-date according to the feature flag state.
...
1. Finalize the rollout plan:
   1. Following the [rollout guidelines](https://docs.gitlab.com/development/feature_flags/controls/#rolling-out-changes) determine a plan for rolling out your feature flag.
...
1. Removing the flag, cleaning up, and feature announcement:
   1. Follow the [feature flag clean up process](https://docs.gitlab.com/development/feature_flags/controls/#cleaning-up).
   1. Ensure that the [feature flag documentation process](https://docs.gitlab.com/development/documentation/feature_flags/) has been followed.
   1. Ensure the flag is removed from the code and from the database with the [chatops feature flag removal command](https://docs.gitlab.com/development/feature_flags/controls/#cleanup-chatops).
...
the feature flag is [enabled **globally** on **production**](https://docs.gitlab.com/development/feature_flags/controls/#enabling-a-feature-for-gitlabcom) for **at least one day**
...
Feature flags must be [documented according to their state (enabled/disabled)](https://docs.gitlab.com/development/documentation/feature_flags/),
