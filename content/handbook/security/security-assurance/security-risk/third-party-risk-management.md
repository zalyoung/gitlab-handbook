...
**PCI Attestation of Compliance** PCI Attestations of Compliance (AoCs) are published as a result of external validation of Self-Assessment Questionnaires (SAQ). This is a point in time assessment of the design and operating effectiveness of controls in place within a service providers environment. An AoC may be leveraged along with documentation supporting responsibility assignment of the requirements (typically seen as a Responsibility Matrix) to provide assurance over a service provider's compliance with the PCI DSS framework, which should be assessed when data may be shared with the third-party as part of the services we are using. 
