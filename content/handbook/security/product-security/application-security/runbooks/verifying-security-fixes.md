...
The review of a fix by an application security engineer is triggered by the engineer implementing the fix. The security engineer that triaged the vulnerability report is responsible for validating the security fixes. As the engineer engaging in the verification, these are the steps that should be followed:
