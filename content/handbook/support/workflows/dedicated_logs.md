...
GitLab Dedicated customers can request [access to application logs](https://docs.gitlab.com/ee/administration/dedicated/configure_instance/#access-to-application-logs).
...
Since GitLab Dedicated uses [Cloud Native Hybrid reference architecture](https://docs.gitlab.com/ee/administration/reference_architectures/10k_users/#cloud-native-hybrid-reference-architecture-with-helm-charts-alternative), searching logs on OpenSearch is a bit different from [Kibana](kibana.md).
...
- `correlation_id`: used to [find relevant log entries by correlation ID](https://docs.gitlab.com/ee/administration/logs/tracing_correlation_id/)
...
When [troubleshooting](https://docs.gitlab.com/ee/user/group/saml_sso/troubleshooting/) unexpected behavior in [instance-wide SAML single sign on (SSO)](https://docs.gitlab.com/ee/integration/saml/) for GitLab Dedicated customers, you can use OpenSearch to retrieve the SAML response.
...
1. In the `params` section, you'll see a JSON object that has a key called `SAMLResponse` (The data in the `value` is the base64-encoded SAML response.)
    - Sometimes this key is too large so the log will only contain the value `truncated`. In this case, you will need to ask the customer [to capture the SAML response](https://docs.gitlab.com/ee/user/group/saml_sso/troubleshooting/#generate-a-saml-response) themselves and send it to you.
...
Read more about [what to look for in the SAML response](https://docs.gitlab.com/ee/user/group/saml_sso/troubleshooting/#saml-debugging-tools).
...
Read more on [troubleshooting Elasticsearch](https://docs.gitlab.com/ee/integration/advanced_search/elasticsearch_troubleshooting/#last-resort-to-recreate-an-index) for potential next steps.
