...
- **Avoid** making the namsepace public.
  - *Instead* create a subgroup or project within your test namespace or a namespace iunique to the customer case and assign them sufficient permissions to invite collaboration.

- **Avoid** granting a customer permanent access.
  - *Instead* set an access expiration date when you invite a customer.

- **Avoid** using access tokens for your main GitLab account: a leak in a test project may not be automatically detected and can be used to traverse sensitive company namepsaces.
