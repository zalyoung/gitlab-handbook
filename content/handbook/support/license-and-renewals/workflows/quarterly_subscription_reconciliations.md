...
Often, a statement of approval will be given in replies to that chatter message on the relevant opportunity.  The approval will come from a leader in sales, typically an area VP.  Hover over the approvers name for details.

When a QSR case is being processed by Deal Desk team, the approval may be on the Salesforce case itself.

- See [this here for more information(/handbook/sales/field-operations/requesting-internal-support/#navigating-an-internal-case)] on SFDC cases

On the SFDC case, look for the fields `Approved` and `Approver`, both of which should be marked when the QSR waiver is approved by the relevant approver.

If no approval is located in the linked chatter, or the SFDC case, inform the requestor of this fact, and do no proceed.
