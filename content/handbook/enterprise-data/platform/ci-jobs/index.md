...
Running this CI job in a merge request pipeline will produce an output report that is added directly to a merge request as a comment from a project bot. This report will summarize the results of the job showing total run time, model counts, what models were executed, and the slowest running models.  This report can be suppressed by addling the {{< label name="Supress Results Report" >}} label to the merge request. 
