...
There's a problem that happens intermittently and causes the run_grants job to run successfully, but not grant any permissions to the role.
In the logs you will see the following entry: 

`INFO:root::rotating_light: upstream objects in OBJECT_NAME did not match upstream grants for ROLE :rotating_light:`

This happens because the best source for existing grants in Snowflake is a table that has a derived state, so occasionally it is missing grants / is not up to date yet.
In this case if re-running the grants job does not help, waiting an hour to re-run it should fix the problem. 
